...
def add_features(df):
    ...
    df_copy["is_dog"] = check_is_dog(animal_type=df_copy["animal_type"])

    # Check if it has a name.
    df_copy["has_name"] = check_has_name(name=df_copy["name"])

    # Get sex.
    sex_upton_outcome = df_copy["sex_upon_outcome"]
    df_copy["sex"] = get_sex(sex_upon_outcome=sex_upton_outcome)

    # Check if neutered.
    df_copy["neutered"] = get_neutered(sex_upon_outcome=sex_upton_outcome)

    # Get hair type.
    df_copy["hair_type"] = get_hair_type(breed=df_copy["breed"])

    # Age in days upon outcome.
    df_copy["days_upon_outcome"] = compute_days_upon_outcome(
        age_upon_outcome=df_copy["age_upon_outcome"]
    )
